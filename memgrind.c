...
//recurssively runs i mallocs and i frees
void DRec(void* p, int i, int j){
        if(j==i){
                return;
        }
        p = malloc(1);
...
//increasing powers of two groups of mallocs followed by frees up to 32 of each
void runD(void* p){
        int i;
        for(i = 1; i <= pow(2, 5); i*=2){
                DRec(p, i, 0);
        }
}
